...
def inexact_linesearch(f, x, s, rho, sigma, tau, chi):
    ...
    def interpolate(alpha_0, alpha_L):  # section 3.11
        return ((alpha_0 - alpha_L)* (alpha_0 - alpha_L) * f_prim_alpha(alpha_L)) / (2 * (f_alpha(alpha_L) - f_alpha(alpha_0) + (alpha_0 - alpha_L)*f_prim_alpha(alpha_L)))
    ...
    while not ((LC(alpha_0, alpha_L) == True and RC(alpha_0, alpha_L) == True)):  # while we don't satisfy LC AND RC CHECK
